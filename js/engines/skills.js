// GENERATED from js/engines/skills.mjs — do not edit; run: npm run build:js
...
const parsePoints = (formula, attrs = {}) => {
  ...
  Object.assign(CoCEngine, {
     ...
     */
    BaseSkills: window.CoCBaseSkills || {},

    Occupations: (window.CoCJobs || []).map(line => {
    ...
    isConcreteSecondarySkill: (skillName) => {
        ...
        if (!def) {
            return (name.includes('：') || name.includes(':')) && !window.CoCEngine.isPlaceholderSecondarySkillName(name);
        }
        if (def.isConcreteSpecialization === true) return true;
        return Array.isArray(def.aliases) && def.aliases.some(alias => {
            const normalized = String(alias || '').trim();
            if (!normalized || window.CoCEngine.isPlaceholderSecondarySkillName(normalized)) return false;
    ...
    isVisibleSkillName: (skillName) => {
        const name = String(skillName || '').trim();
        if (!name || window.CoCEngine.isPlaceholderSecondarySkillName(name)) return false;
        const def = window.CoCEngine.BaseSkills[name];
        if (def && def.requiresSpecialization && !window.CoCEngine.isConcreteSecondarySkill(name)) return false;
        if (def && def.isParent && !window.CoCEngine.isConcreteSecondarySkill(name) && def.requiresSpecialization !== false) return false;
        return true;
    },

    getVisibleSkillNames: (extraSkills = []) => {
        const names = [...Object.keys(window.CoCEngine.BaseSkills || {}), ...(extraSkills || [])];
        const result = [];
        for (const rawName of names) {
            const name = String(rawName || '').trim();
            if (name && !result.includes(name) && window.CoCEngine.isVisibleSkillName(name)) result.push(name);
        }
        return result;
    },
    ...
    isChildOfParentSkill: (skillName, parentName) => {
        ...
        const parentDef = window.CoCEngine.BaseSkills[parent];
        const skillDef = window.CoCEngine.BaseSkills[skill];
        ...
        const aliasParentDef = parentDef || Object.values(window.CoCEngine.BaseSkills || {}).find(def =>
    ...
    isClassSkillName: (skillName, classSkillsString = '') => {
        ...
        for (const parentName of classSkills) {
            if (window.CoCEngine.isChildOfParentSkill(skill, parentName)) return true;
        }
        return false;
    },
    ...
    getSkillValue: (char, skillName) => {
        ...
        const baseSkills = window.CoCEngine.BaseSkills;
