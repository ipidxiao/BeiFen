// GENERATED from js/engines/mythos.mjs — do not edit; run: npm run build:js
window.CoCEngine = window.CoCEngine || {};
window.CoCEngine.MythosEngine = {
    ...
    initialBrowse: function(ch, tomeKey) {
        ...
        var sanPass = window.CoCEngine.parseDice(parts[0] || '1D6');
        var sanFail = window.CoCEngine.parseDice(parts[1] || '1D10');
        ...
        var mythosGain = window.CoCEngine.parseDice(tome.mythosGainInitial);
        // Apply SAN loss
    ...
    fullStudy: function(ch, tomeKey, weeksSpent) {
        ...
        if (completed) {
            ...
            var sanPass = window.CoCEngine.parseDice(parts[0] || '1D10');
            var sanFail = window.CoCEngine.parseDice(parts[1] || '2D10');
            var sanRoll = Math.floor(Math.random() * 100) + 1;
            var passed = sanRoll <= (ch.derived ? ch.derived.sanity : (ch.sanity || 50));
            var sanLoss = passed ? sanPass : sanFail;
            var mythosGain = window.CoCEngine.parseDice(tome.mythosGainFull);
            if (ch.derived) ch.derived.sanity = Math.max(0, (ch.derived.sanity || 0) - sanLoss);
            else ch.sanity = Math.max(0, (ch.sanity || 0) - sanLoss);
            if (!ch.skills) ch.skills = {};
            ch.skills['克苏鲁神话'] = (ch.skills['克苏鲁神话'] || 0) + mythosGain;
    ...
    castSpell: function(ch, spellName, target) {
        ...
        // Deduct SAN
        var sanCost = window.CoCEngine.parseDice(spell.sanCost);
