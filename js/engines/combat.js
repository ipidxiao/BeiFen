// GENERATED from js/engines/combat.mjs — do not edit; run: npm run build:js
window.CoCEngine = window.CoCEngine || {};
window.CoCEngine.CombatEngine = {
    /**
     * 计算伤害
     */
    calculateDamage: function(damageStr, attackerDB, targetArmor = 0) {
        let totalDamage = window.CoCEngine.parseDice(damageStr);
        if (attackerDB) totalDamage += window.CoCEngine.parseDice(attackerDB);
    ...
    resolveCombatExchange: function(attacker, defender, options = {}) {
        ...
        const attackRes = window.CoCEngine.checkSkill(attackSkill, attacker, difficulty);
        ...
        if (options.action === 'counter') {
            const counterRes = window.CoCEngine.checkSkill(options.counterSkill || '斗殴', defender);
            ...
            const dodgeRes = window.CoCEngine.checkSkill('闪避', defender);
    ...
    resolveBurstFire: function(attacker, defender, weapon, rounds, mode) {
        ...
        for (var i = 0; i < actualRounds; i++) {
            ...
            var ar = window.CoCEngine.checkSkill(weapon.skill || '手枪', attacker, difficulty);
            var dr = window.CoCEngine.checkSkill('闪避', defender, 'normal');
    ...
    checkImpale: function(weapon, attackResult) {
        ...
        if (attackResult.level === '极难成功' || attackResult.level === '大成功') {
            var d = window.CoCEngine.parseDice(weapon.damage || '1D6');
    ...
    checkMalfunction: function(weapon, attackResult) {
        ...
        if (attackResult.level === '大失败') {
            ...
            const malRoll = (window.CoCEngine._rollD100 && typeof window.CoCEngine._rollD100 === 'function')
                ? window.CoCEngine._rollD100()
