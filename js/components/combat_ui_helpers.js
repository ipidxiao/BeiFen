// GENERATED from js/components/combat_ui_helpers.mjs — do not edit; run: npm run build:js
// Pure combat UI helpers — shared by StoryCombat and Node/jsdom tests.

const COMBAT_QUICK_ACTIONS = Object.freeze([
...
function filterActiveInitiativeOrder(initiativeOrder, enemies, roster) {
...
function computeActiveTurnIdx(currentTurnIdx, activeOrderLength) {
    const len = Math.max(1, Number(activeOrderLength) || 0);
    const idx = Number(currentTurnIdx) || 0;
    return idx % len;
}

function resolveFirstWoundedChar(roster, activeChars) {
    const chars = Array.isArray(roster) ? roster : [];
...
function buildQuickActionText(actionId, woundedName = '队友') {
