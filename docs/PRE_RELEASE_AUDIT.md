...
[x] node scripts/ci_smoke.mjs → build:js 无 drift + SW CACHE_NAME 一致性 + exports（本批次）
...
```
[ ] python build.py 出包（自动 build:js + 注入 SW CACHE_NAME 内容哈希）
[ ] 确认 vendor/ 与 8 个 packages/*.json 已入 ZIP
[ ] vendor 版本核对：vue.global.prod.js · bootstrap.min.css · chart.min.js · pdf.min.js · pdf.worker.min.js 与 package/文档记录一致
```

### 人工 QA — 无障碍 / 离线 / PWA（须发布前人工执行，勿勾选为已完成除非已实测）

```
[ ] 键盘 Tab 走查（全程仅用 Tab / Shift+Tab / Enter / Space，不用鼠标）：
      大厅 → 设置/模组库入口 → 建卡各步 → 叙事输入与骰子 → 进入战斗面板 → 存档槽 → 读档恢复
[ ] 窄屏目测：≤390px 宽度下大厅、建卡、战斗、设置无横向溢出、主按钮可触达
[ ] 离线刷新：npx serve . → 首启联网加载 → DevTools 勾选 Offline → 硬刷新 → 骰子/战斗/手册/本地剧本仍可用
[ ] PWA 添加主屏：Chrome/Edge「安装应用」或 iOS Safari「添加到主屏幕」→ 从图标冷启动 → 核心 UI 正常
[ ] 首启无 API Key：断网或清 Key 后确认「📴 离线模式」提示与 AI 禁用说明可见
[ ] 载入旧版本存档 fixture 验证迁移无损（v1–v7）
```

### 发布说明

```
[ ] 撰写 CHANGELOG（以 js/data/dev_logs 为准；wontfix 见 RELEASE_NOTES_DRAFT.md）
```
