...
> ⚠️ **出包门禁**：**禁止手工改 `.js` 或手编 ZIP**。发布前必须使用 **`python build.py`**（自动 `npm run build:js` + 注入 `sw.js` 的 `CACHE_NAME` 内容哈希）。否则 PWA 离线缓存不会失效，用户会拿到旧版本静态资源。

> 审计日期：2026-07-06 · 版本 **18.1.0** · 性质：只读顾问审计（condensed from pre-release audit session）  
> 关联文档：[ROADMAP.md](./ROADMAP.md) · [AUDIT6_BATCHES.md](./AUDIT6_BATCHES.md) · [OPTIONAL_ENHANCEMENTS.md](./OPTIONAL_ENHANCEMENTS.md) · [RELEASE_NOTES_DRAFT.md](./RELEASE_NOTES_DRAFT.md)
...
> ⚠️ 出包：**仅** `python build.py`（勿手改 `sw.js` / 浏览器 `.js` 或手工 ZIP）。

### 自动化门禁（CI）

```
[x] node tests/run_all_smoke.js → 44/44 PASS（2026-07-07 复跑）
[x] node scripts/ci_smoke.mjs → build:js 无 drift + SW CACHE_NAME 一致性 + exports（2026-07-07 复跑）
...
```
[x] python build.py 出包（自动 build:js + 注入 SW CACHE_NAME 内容哈希）— 2026-07-07 复跑 → CoC_Engine_V18.1.zip（4062KB, 229 files）
[x] 确认 vendor/ 与 8 个 packages/*.json 已入 ZIP — 2026-07-07 自动化校验：vendor 5 文件 + packages 8 JSON 均在 ZIP 内
[x] SW CACHE_NAME 一致性 — ci:smoke verify_sw_cache → coc-engine-v18.1.0-24142195 OK
[ ] vendor 版本核对：vue.global.prod.js · bootstrap.min.css · chart.min.js · pdf.min.js · pdf.worker.min.js 与 package/文档记录一致（需人工）
```

### 人工 QA — 无障碍 / 离线 / PWA（须发布前人工执行，勿勾选为已完成除非已实测）

> 以下项**需人工**在浏览器中实测，自动化无法替代。

```
[ ] 键盘 Tab 走查（需人工）— 全程仅用 Tab / Shift+Tab / Enter / Space，不用鼠标：
      大厅 → 设置/模组库入口 → 建卡各步 → 叙事输入与骰子 → 进入战斗面板 → 存档槽 → 读档恢复
[ ] 窄屏目测（需人工）：≤390px 宽度下大厅、建卡、战斗、设置无横向溢出、主按钮可触达
[ ] 离线刷新（需人工）：npx serve . → 首启联网加载 → DevTools 勾选 Offline → 硬刷新 → 骰子/战斗/手册/本地剧本仍可用
[ ] PWA 添加主屏（需人工）：Chrome/Edge「安装应用」或 iOS Safari「添加到主屏幕」→ 从图标冷启动 → 核心 UI 正常
[ ] 首启无 API Key（需人工）：断网或清 Key 后确认「📴 离线模式」提示与 AI 禁用说明可见
[ ] 载入旧版本存档 fixture 验证迁移无损（v1–v7）— Node smoke 已覆盖；真机读档仍建议人工 spot-check
```

### 发布说明

```
[x] 撰写 CHANGELOG — 见 docs/RELEASE_NOTES_DRAFT.md v18.1.0 节（2026-07-07）
```
